...
# -----------------------------------------------------------------------------
# К4: парсер JSON в llm.py корректно обрабатывает JSON + хвостовую prose с }
# -----------------------------------------------------------------------------


def test_llm_parse_json_handles_trailing_prose_with_brace() -> None:
    """Жадный regex ловил бы до последней } в тексте. ``raw_decode`` —
    корректный парсинг до баланса скобок.

    К4 фикс-тест: убеждаемся, что JSON после текста с закрывающей
    скобкой парсится как первый объект, а не как обрезок до последней }.
    """
    from workspace.skills.audit_formulation_strengthener.scripts.llm import (
        _parse_json,
    )
    ...
    # JSON объект + хвостовая prose с лишней } в тексте.
    text = '{"key": "value"} and then some text with } in it.'
    obj = _parse_json(text)
    assert obj == {"key": "value"}

    # JSON с вложенным объектом и prose после.
    text2 = '{"outer": {"inner": 1}, "ok": true} trailing text with } brace'
    obj2 = _parse_json(text2)
    assert obj2 == {"outer": {"inner": 1}, "ok": True}

    # ```-fenced JSON + prose после.
    text3 = '```json\n{"fenced": 1}\n```\n then prose with } brace'
    obj3 = _parse_json(text3)
    assert obj3 == {"fenced": 1}
